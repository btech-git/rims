...
<div class="small-12 columns">
        ...
        <div class="clearfix page-action">
            ...
            <div class="row">
                <div class="large-12 columns">
                    <?php echo CHtml::link('<span class="fa fa-list"></span>Manage Registration', Yii::app()->baseUrl . '/frontEnd/generalRepairRegistration/admin', array('class' => 'button cbutton left', 'style' => 'margin-right:10px', 'visible' => Yii::app()->user->checkAccess("generalRepairCreate") || Yii::app()->user->checkAccess("generalRepairEdit"))) ?>
                    ...
                    <?php if ($model->status !== 'Finished' && $model->status !== 'CANCELLED!!!'): ?>
                        ...
                        <?php if (Yii::app()->user->checkAccess("generalRepairEdit")): ?>
                            <?php echo CHtml::link('<span class="fa fa-edit"></span>Edit Customer Data', Yii::app()->baseUrl . '/frontEnd/generalRepairRegistration/update?id=' . $model->id, array('class' => 'button cbutton left', 'style' => 'margin-right:10px', 'visible' => Yii::app()->user->checkAccess("generalRepairEdit"))) ?>
                        <?php endif; ?>
                    
                        <?php if (count($invoices) == 0): ?>
                            <?php echo CHtml::link('<span class="fa fa-plus"></span>Product & Service', Yii::app()->baseUrl . '/frontEnd/generalRepairRegistration/addProductService?registrationId=' . $model->id, array('class' => 'button success left', 'style' => 'margin-right:10px', 'visible' => Yii::app()->user->checkAccess("generalRepairCreate") || Yii::app()->user->checkAccess("generalRepairEdit"))) ?>
                        <?php endif; ?>
                        ...
                        <?php if (count($model->registrationServices) > 0): ?>
                            <?php echo CHtml::link('<span class="fa fa-plus"></span>Generate Work Order', Yii::app()->baseUrl . '/frontEnd/generalRepairRegistration/generateWorkOrder?id=' . $model->id, array(
    ...
    <div>
        <?php if (Yii::app()->user->checkAccess("generalRepairSupervisor") && $model->status !== 'Finished' && $model->status !== 'CANCELLED!!!'): ?>
            <?php echo CHtml::link('<span class="fa fa-minus"></span>Cancel Transaction', array("/frontEnd/generalRepairRegistration/cancel", "id" => $model->id), array(
                'class' => 'button alert', 
                'style' => 'margin-right:10px', 
            )); ?>
        <?php endif; ?>
        ...
        <?php if (empty($model->work_order_number) && $model->status !== 'Pending' && empty($model->sales_order_number)): ?>
            <?php echo CHtml::link('<span class="fa fa-bookmark"></span>Pending', Yii::app()->baseUrl.'/frontEnd/generalRepairRegistration/pendingOrder?id=' . $model->id, array('class'=>'button secondary right', 'style' => 'margin-right:10px')) ?>
        <?php endif; ?>
        <?php if (!empty($model->work_order_number) && $model->total_service > 0): ?>
            <?php echo CHtml::link('<span class="fa fa-print"></span>Print Work Order', Yii::app()->baseUrl.'/frontEnd/generalRepairRegistration/pdfWorkOrder?id=' . $model->id, array('class'=>'button warning right', 'style' => 'margin-right:10px', 'target' =>'_blank')) ?>
        <?php endif; ?>
        <?php if (!empty($model->sales_order_number) && $model->status !== 'Finished'): ?>
            <?php echo CHtml::link('<span class="fa fa-print"></span>Print Sales Order', Yii::app()->baseUrl.'/frontEnd/generalRepairRegistration/pdfSaleOrder?id=' . $model->id, array('class'=>'button warning right', 'style' => 'margin-right:10px', 'target' =>'_blank')) ?>
        <?php endif; ?>
        <?php if ($model->status !== 'Finished'): ?>
            <?php echo CHtml::link('<span class="fa fa-print"></span>Print Estimasi', Yii::app()->baseUrl.'/frontEnd/generalRepairRegistration/pdf?id=' . $model->id, array('class'=>'button warning right', 'style' => 'margin-right:10px', 'target' =>'_blank')) ?>
        <?php endif; ?>
    </div>
</div>
