<?php if (count($bodyRepairRegistration->serviceDetails) > 0): ?>
	<table>
		<thead>
			<tr>
				<th>Service name</th>
		...
		<tbody>
			<?php foreach ($bodyRepairRegistration->serviceDetails as $i => $serviceDetail): ?>
                    ...
                    <tr>
                        ...
                        <td><?php echo CHtml::activeTextField($serviceDetail,"[$i]price", array(
                                'onchange'=>CHtml::ajax(array(
                                    'type'=>'POST',
                                    'dataType'=>'JSON',
                                    'url'=>CController::createUrl('ajaxJsonTotalService', array('id'=>$bodyRepairRegistration->header->id, 'index'=>$i)),
                        ...
                        <td><?php echo CHtml::activeDropDownList($serviceDetail,"[$i]discount_type", array('Nominal' => 'Nominal',
                                'Percent' => '%'), array('prompt'=>'[--Select Discount Type--]')); ?></td>
                        <td>
                            <?php echo CHtml::activeTextField($serviceDetail,"[$i]discount_price", array('readonly'=>$serviceDetail->is_quick_service == 1 ? true : false,
                                'onchange'=>CHtml::ajax(array(
                                    'type'=>'POST',
                                    'dataType'=>'JSON',
                                    'url'=>CController::createUrl('ajaxJsonTotalService', array('id'=>$bodyRepairRegistration->header->id, 'index'=>$i)),
                        ...
                        <td>
                            <?php echo CHtml::button('X', array(
                                'onclick' => CHtml::ajax(array(
                                    'type' => 'POST',
                                    'url' => CController::createUrl('ajaxHtmlRemoveServiceDetail', array('id' => $bodyRepairRegistration->header->id, 'index' => $i)),
                                    'update' => '#service',
                                )),
                            )); ?>
                        </td>
