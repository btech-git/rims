...
<div class="small-12 columns">
        ...
        <div class="clearfix page-action">
            ...
            <div class="row">
                <div class="large-12 columns">
                    <?php echo CHtml::link('<span class="fa fa-list"></span>Manage', Yii::app()->baseUrl . '/frontEnd/bodyRepairRegistration/admin', array('class' => 'button cbutton left', 'style' => 'margin-right:10px', 'visible' => Yii::app()->user->checkAccess("frontEnd.bodyRepairRegistration.admin"))) ?>

                    <?php if ($model->status !== 'Finished' && $model->status !== 'CANCELLED!!!'): ?>
                        <?php if (Yii::app()->user->checkAccess("bodyRepairEdit")): ?>
                            <?php echo CHtml::link('<span class="fa fa-edit"></span>Edit Registration', Yii::app()->baseUrl . '/frontEnd/bodyRepairRegistration/update?id=' . $model->id, array('class' => 'button cbutton left', 'style' => 'margin-right:10px', 'visible' => Yii::app()->user->checkAccess("frontEnd.bodyRepairRegistration.update"))) ?>
                        <?php endif; ?>
                        <?php if (count($invoices) == 0): ?>
                            <?php echo CHtml::link('<span class="fa fa-plus"></span>Product & Service', Yii::app()->baseUrl . '/frontEnd/bodyRepairRegistration/addProductService?registrationId=' . $model->id, array('class' => 'button success left', 'style' => 'margin-right:10px', 'visible' => Yii::app()->user->checkAccess("frontEnd.bodyRepairRegistration.create"))) ?>
                        <?php endif; ?>
                        ...
                        <?php if (count($model->registrationServices) > 0): ?>
                            <?php echo CHtml::link('<span class="fa fa-plus"></span>Generate Work Order', Yii::app()->baseUrl . '/frontEnd/bodyRepairRegistration/generateWorkOrder?id=' . $model->id, array(
    ...
    <div>
        <?php if (Yii::app()->user->checkAccess("bodyRepairSupervisor") && $model->status !== 'Finished' && $model->status !== 'CANCELLED!!!'): ?>
            <?php echo CHtml::link('<span class="fa fa-minus"></span>Cancel Transaction', array("/frontEnd/bodyRepairRegistration/cancel", "id" => $model->id), array(
        ...
        <?php if (empty($model->work_order_number) && $model->status !== 'Pending' && empty($model->sales_order_number)): ?>
            <?php echo CHtml::link('<span class="fa fa-bookmark"></span>Pending', Yii::app()->baseUrl.'/frontEnd/bodyRepairRegistration/pendingOrder?id=' . $model->id, array('class'=>'button secondary right', 'style' => 'margin-right:10px')); ?>
        <?php endif; ?>
        <?php if (!empty($model->work_order_number) && $model->total_service > 0): ?>
            <?php echo CHtml::link('<span class="fa fa-print"></span>Print Work Order', Yii::app()->baseUrl.'/frontEnd/bodyRepairRegistration/pdfWorkOrder?id=' . $model->id, array('class'=>'button warning right', 'style' => 'margin-right:10px')); ?>
        <?php endif; ?>
        <?php if (!empty($model->sales_order_number) && $model->status !== 'Finished'): ?>
            <?php echo CHtml::link('<span class="fa fa-print"></span>Print Sales Order', Yii::app()->baseUrl.'/frontEnd/bodyRepairRegistration/pdfSaleOrder?id=' . $model->id, array('class'=>'button warning right', 'style' => 'margin-right:10px')); ?>
        <?php endif; ?>
        <?php if ($model->status !== 'Finished'): ?>
            <?php echo CHtml::link('<span class="fa fa-print"></span>Print Estimasi', Yii::app()->baseUrl.'/frontEnd/bodyRepairRegistration/pdf?id=' . $model->id, array('class'=>'button warning right', 'style' => 'margin-right:10px')); ?>
        <?php endif; ?>
    </div>
</div>
