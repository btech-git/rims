<?php if (count($bodyRepairRegistration->damageDetails) > 0): ?>
	<table>
	<thead>
		<tr>
			<th>Service</th>
	...
	<tbody>
		<?php foreach ($bodyRepairRegistration->damageDetails as $i => $damageDetail): ?>
			<tr>
				<td> <?php echo CHtml::activeHiddenField($damageDetail, "[$i]service_id", array('size'=>20,'maxlength'=>20)); ?>
					...
					<?php echo CHtml::button('add to Service', array(
                        ...
                        'onclick' => '
                            ...
                            type: "POST",
                            //dataType: "JSON",
                            url: "' . CController::createUrl('ajaxHtmlAddServiceInsuranceDetail', array('id'=> $bodyRepairRegistration->header->id)).'&serviceId="+serviceId+"&insuranceId="+ $("#RegistrationTransaction_insurance_company_id").val()+"&damageType="+ damageType+"&repair="+ $("#RegistrationTransaction_repair_type").val(),
                            data:$("form").serialize(),
				...
				<td>
                    <?php echo CHtml::button('X', array(
				     	'onclick' => CHtml::ajax(array(
					       	'type' => 'POST',
					       	'url' => CController::createUrl('ajaxHtmlRemoveDamageDetail', array('id' => $bodyRepairRegistration->header->id, 'index' => $i)),
					       	'update' => '#damage',
			      		)),
			     	)); ?>
                </td>
