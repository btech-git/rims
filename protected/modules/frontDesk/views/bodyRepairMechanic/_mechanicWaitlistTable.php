    ...
    <tbody>
        <?php foreach ($modelDataProvider->data as $model): ?>
            <tr>
                ...
                <td><?php echo $model->totalTimeFormatted; ?></td>
                <td><?php echo $model->status != null ? $model->service_status : '-'; ?></td>
                <td><?php echo $model->getPriorityLiteral($model->priority_level); ?></td>
                <td><?php echo CHtml::link('<span class="fa fa-wrench"></span>Detail', Yii::app()->createUrl("frontDesk/bodyRepairMechanic/viewDetailWorkOrder", array("registrationId"=>$model->id)), array('class' => 'button warning', 'target' => '_blank')); ?></td>
            </tr>
        <?php endforeach; ?>
    </tbody>
</table>