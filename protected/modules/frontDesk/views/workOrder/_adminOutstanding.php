    ...
    <tbody>
        <?php foreach ($workOrderSummary->dataProvider->data as $header): ?>
            <tr class="items1">
                ...
                <th class="width1-6">
                    <?php echo CHtml::link($header->work_order_number, array("/frontDesk/registrationTransaction/view", "id"=>$header->id), array('target' => '_blank')); ?>
                </th>
                <th class="width1-7">
                    <?php echo CHtml::encode(Yii::app()->dateFormatter->format("d MMM yyyy", CHtml::value($header, 'work_order_date'))); ?>
                </th>
