<?php if (count($registrationTransaction->damageDetails) > 0): ?>
	<table>
	<thead>
		<tr>
			<th>Service</th>
	...
	<tbody>
		<?php foreach ($registrationTransaction->damageDetails as $i => $damageDetail): ?>
			<tr>
				<td> <?php echo CHtml::activeHiddenField($damageDetail, "[$i]service_id", array('size'=>20,'maxlength'=>20)); ?>
					...
					<?php echo CHtml::activeTextField($damageDetail,"[$i]service_name",
						array(
							'size'=>15,
							'maxlength'=>10,
							'readonly'=>true,
					...
					<?php echo CHtml::button('add to Service', array(
                        ...
                        'onclick' => '
                            ...
                            type: "POST",
                            //dataType: "JSON",
                            url: "' . CController::createUrl('ajaxHtmlAddServiceInsuranceDetail', array('id'=> $registrationTransaction->header->id)).'&serviceId="+serviceId+"&insuranceId="+ $("#RegistrationTransaction_insurance_company_id").val()+"&damageType="+ damageType+"&repair="+ $("#RegistrationTransaction_repair_type").val(),
                            data:$("form").serialize(),
				...
				<td>
					<?php if($damageDetail->service_id != ""){
						$first = true;
							$rec = "";
				...
				<td><?php
				    echo CHtml::button('X', array(
				     	'onclick' => CHtml::ajax(array(
					       	'type' => 'POST',
					       	'url' => CController::createUrl('ajaxHtmlRemoveDamageDetail', array('id' => $registrationTransaction->header->id, 'index' => $i)),
					       	'update' => '#damage',
			      		)),
			     	));
		     	?></td>
