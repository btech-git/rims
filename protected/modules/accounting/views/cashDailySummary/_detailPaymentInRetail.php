        ...
        <?php foreach ($paymentInRetailList as $paymentInRetailItem): ?>
            ...
            <tr>
                <?php foreach ($paymentInRetailItem as $i => $paymentInRetail): ?>
                    <td style="text-align: right"><?php echo CHtml::encode(Yii::app()->numberFormatter->format('#,##0', $paymentInRetail)); ?></td>
                    <?php if ($i > 0): ?>
                        <?php $total += $paymentInRetail; ?>
                    <?php endif; ?>
                <?php endforeach; ?>
                <td style="text-align: right; font-weight: bold"><?php echo CHtml::encode(Yii::app()->numberFormatter->format('#,##0', $total)); ?></td>
            </tr>
        <?php endforeach; ?>
    </tbody>
</table>