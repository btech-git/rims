    ...
    <fieldset>
        ...
        <table class="report">
            ...
            <tbody>
                ...
                <?php $purchaseTransactions = JurnalUmum::model()->findAllByAttributes(array(
                    'kode_transaksi' => $model->transaction_number, 
                    'is_coa_category' => 0
                )); ?>
                <?php foreach ($purchaseTransactions as $i => $header): ?>
                ...
                <?php $depreciationransactions = JurnalUmum::model()->findAllByAttributes(array(
                    'transaction_subject' => $model->transaction_number, 
                    'transaction_type' => 'DFA',
                    'is_coa_category' => 0
                )); ?>
                <?php foreach ($depreciationransactions as $i => $header): ?>
                    <?php $amountDebit = $header->debet_kredit == 'D' ? CHtml::value($header, 'total') : 0; ?>
                    <?php $amountCredit = $header->debet_kredit == 'K' ? CHtml::value($header, 'total') : 0; ?>
                ...
                <?php if (!empty($model->assetSales)): ?>
                    ...
                    <?php foreach ($saleTransactions as $saleTransaction): ?>
                        <?php $saleJournals = JurnalUmum::model()->findAllByAttributes(array(
                            'transaction_subject' => $model->transaction_number, 
                            'transaction_type' => 'SFA',
                            'is_coa_category' => 0,
                        )); ?>
                        <?php foreach ($saleJournals as $i => $saleJournal): ?>
                            ...
                            <tr>
                                <td style="text-align: center"><?php echo $count + 1; ?></td>
                                <td><?php echo CHtml::encode(CHtml::value($saleJournal, 'branchAccountCode')); ?></td>
                                <td><?php echo CHtml::encode(CHtml::value($saleJournal, 'branchAccountName')); ?></td>
                                <td><?php echo CHtml::encode(CHtml::value($saleJournal, 'kode_transaksi')); ?></td>
