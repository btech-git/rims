...
class BranchController extends Controller
{
	...
	{
		//$branchWarehouses = BranchWarehouse::model()->findAllByAttributes(array('branch_id'=>$id));
		...
		//$branchEmployees = EmployeeBranchDivisionPositionLevel::model()->findAllByAttributes(array('branch_id'=>$id));
	  ...
	  $divisionChecks = DivisionBranch::model()->findAllByAttributes(array('branch_id'=>$id));
		...
		if(isset($_POST['Branch']))
		{
			...
			if ($branch->save(Yii::app()->db)){
				...
				//foreach ($branch->positions as $key => $position) {
