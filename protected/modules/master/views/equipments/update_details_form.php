...
<h1><?php echo "Update Equipment Details"; ?></h1>
<?php $form=$this->beginWidget('CActiveForm', array(
 ...
 ?>	
	...
	<?php echo $form->errorSummary($equipmentDetails->header); ?>
	<?php echo $form->errorSummary($equipmentDetails->equipmentMaintenances); ?>
	<div class="row">
		<div class="small-12 medium-6 columns">   
			...
			<div class="field">
				<div class="row collapse">
					...
					<label class="prefix">
						<?php echo $form->labelEx($equipmentDetails->header,'equipment_id'); ?>						
					</div>
					<div class="small-8 columns">
						<?php //echo $form->textField($model,'equipment_id'); ?>
						<?php echo $form->dropDownList($equipmentDetails->header, 'equipment_id', CHtml::listData(Equipments::model()->findAll(), 'id', 'name'),array(
										'prompt' => '[--Select Equipments--]'));?>
						<?php echo $form->error($equipmentDetails->header,'equipment_id'); ?>
					</div>
				</div>
			 </div>
			...
			<div class="field">
				<div class="row collapse">
					<div class="small-4 columns">
					<label class="prefix">
						<?php echo $form->labelEx($equipmentDetails->header,'equipment_code'); ?>
					</div>
					<div class="small-8 columns">
						<?php echo $form->textField($equipmentDetails->header,'equipment_code',array('size'=>60,'maxlength'=>100)); ?>
						<?php echo $form->error($equipmentDetails->header,'equipment_code'); ?>
					</div>
				</div>
			</div>	
			...
			<div class="field">
				<div class="row collapse">
					<div class="small-4 columns">
					  <label class="prefix"><?php echo $form->labelEx($equipmentDetails->header,'brand'); ?></label>
					</div>
					<div class="small-8 columns">
						<?php echo $form->textField($equipmentDetails->header,'brand',array('size'=>30,'maxlength'=>30)); ?>
						<?php echo $form->error($equipmentDetails->header,'brand'); ?>
					</div>
				</div>
			</div>		
			...
			<div class="field">
				<div class="row collapse">
					<div class="small-4 columns">
					  <label class="prefix"><?php echo $form->labelEx($equipmentDetails->header,'purchase_date'); ?></label>
					</div>
					<div class="small-8 columns">
						<?php $this->widget('zii.widgets.jui.CJuiDatePicker',array(
											'model' => $equipmentDetails->header,
						...
						<?php echo $form->error($equipmentDetails->header,'purchase_date'); ?>
					</div>
				</div>
			 </div>		 
			 ...
			 <div class="field">
				<div class="row collapse">
					<div class="small-4 columns">
					  <label class="prefix"><?php echo $form->labelEx($equipmentDetails->header,'age'); ?></label>
					</div>
					<div class="small-8 columns">
						<?php echo $form->textField($equipmentDetails->header,'age',array('placeHolder'=>'how many days old','readOnly'=>'readOnly')); ?>
						<?php echo $form->error($equipmentDetails->header,'age'); ?>
					</div>
				</div>
			 </div>		 
   ...
   <hr/>
   	...
   	<!-- Task form-->
			...
			<?php 
				...
				foreach($detailEquipments as  $j => $branches){
					$equipmentDetailsTasks[$j] = equipmentTask::model()->findAllByAttributes(array('equipment_id'=>$branches->id)); 
				}
				// end of get tasks of all same branch related equipments
				/*echo"<pre>";
				echo count($equipmentDetailsTasks); 
				print_r($equipmentDetailsTasks); 
				//exit;*/
				foreach($equipmentDetailsTasks as  $j => $equipmentTasks): 
				//	echo"<pre>";
		   ...
		   <div class="row collapse">
			...
			if($maintenanceCount==0)
			{
				...
				<table class="detail">
					...
					$this->renderPartial('add_maintenance', array('equipmentDetails'=>$equipmentDetails,'equipmentTasks'=>$equipmentTasks,'j'=>$k,'task_id'=>$current_task_id,'equipment_id'=>$task_id['equipment_id']), false, true); 
					$k = $k+1;
				 }?>
					 </table>			
			...
			?>
				<table class="detail">
					...
					<th>Condition </th>
					<th>Checked </th>
					<?php $this->renderPartial('_detail_maintenance_form', array('equipmentDetails'=>$equipmentDetails));?>
				</table>
				</div>
				</div>		
			</div>
					...
					<?php
						...
						$this->widget('application.extensions.fullcalendar.EFullCalendarHeart', array(
							...
							'options'=>array(
								...
								'events'=>$this->createUrl('equipmentDetails/getEvents?id='.$equipmentDetails->header->id), // URL to get event
							)));
	...
	<div class="field buttons text-center">
		<?php echo CHtml::submitButton($equipmentDetails->header->isNewRecord ? 'Create' : 'Save', array('class'=>'button cbutton')); ?>
	</div>
	<?php $this->endWidget(); ?>
</div><!-- form -->
</div>	