...
class EquipmentDetail extends CComponent
{
	public $header;
	...
	{
		$maintainanceDetail = new EquipmentMaintenances();
	...
	public function validate()
	{
		...
		{
			foreach ($this->equipmentMaintenances as $equipmentMaintenances)
			{
				$fields = array('equipment_task_id','employee_id','maintenance_date','next_maintenance_date','check_date');
				$valid = $equipmentMaintenances->validate($fields) && $valid;
			}
		}
		else {
			$valid = true;
	...
	public function flush()
	{
		...
		//maintenance
		$equipmentMaintenances = EquipmentMaintenances::model()->findAllByAttributes(array('equipment_detail_id'=>$this->header->id));
		$maintenanceId = array();
		...
		{
			$maintainanceDetail->equipment_detail_id = $this->header->id;			
		...
		if($delete_maintenance_array != NULL)
		{
			...
			EquipmentMaintenances::model()->deleteAll($maintainancecriteria);
		}		
