...
class CompanySaleByProductCategoryServiceTypeController extends Controller {
    ...
    protected function saveToExcel($saleReportData, array $options = array()) {
        ...
        $worksheet->getStyle('A1:Z3')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
        ...
        foreach ($companyList as $companyItem) {
            $worksheet->setCellValue("A{$rowCounter}", CHtml::value($companyItem, 'name'));
            $rowCounter++;
            $columnCounter = 'B';
            ...
            foreach ($branchList as $branchItem) {
                if ($branchItem->company_id === $companyItem->id) {
