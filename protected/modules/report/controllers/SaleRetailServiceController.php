...
class SaleRetailServiceController extends Controller {
    ...
    protected function saveToExcel($saleRetailSummary, $branchId, $dataProvider, array $options = array()) {
        ...
        foreach ($dataProvider->data as $header) {
            ...
            $worksheet->setCellValue("J{$counter}", CHtml::encode(CHtml::value($header, 'service.name')));
            $worksheet->setCellValue("K{$counter}", CHtml::encode(CHtml::value($header, 'claim')));
            $worksheet->setCellValue("L{$counter}", CHtml::encode(CHtml::value($header, 'price')));
            $worksheet->setCellValue("M{$counter}", CHtml::encode(CHtml::value($header, 'discount_price')));
            $worksheet->setCellValue("N{$counter}", CHtml::encode(CHtml::value($header, 'total_price')));
