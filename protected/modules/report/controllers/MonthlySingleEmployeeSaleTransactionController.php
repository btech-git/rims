...
class MonthlySingleEmployeeSaleTransactionController extends Controller {
    ...
    public function filterAccess($filterChain) {
        if ($filterChain->action->id === 'summary') {
            if (!(Yii::app()->user->checkAccess('monthlySaleFrontReport'))) {
    ...
    public function actionSummary() {
        ...
        $monthlySingleEmployeeSaleReport = InvoiceHeader::getMonthlySingleEmployeeSaleReport($year, $month, $employeeId);
        
        $monthlySingleEmployeeSaleProductReport = InvoiceDetail::getMonthlySingleEmployeeSaleProductReport($year, $month, $employeeId);
        
        $monthlySingleEmployeeSaleReportData = array();
        foreach ($monthlySingleEmployeeSaleReport as $monthlySingleEmployeeSaleReportItem) {
            $monthlySingleEmployeeSaleReportData[$monthlySingleEmployeeSaleReportItem['day']] = $monthlySingleEmployeeSaleReportItem;
        }
        
        $monthlySingleEmployeeSaleProductReportData = array();
        foreach ($monthlySingleEmployeeSaleProductReport as $monthlySingleEmployeeSaleProductReportItem) {
            $monthlySingleEmployeeSaleProductReportData[$monthlySingleEmployeeSaleProductReportItem['day']] = $monthlySingleEmployeeSaleProductReportItem;
        }
        ...
        if (isset($_GET['SaveExcel'])) {
            $this->saveToExcel($monthlySingleEmployeeSaleReportData, $monthlySingleEmployeeSaleProductReportData, $month, $year, $employeeId);
        }
        
        $this->render('summary', array(
            'monthlySingleEmployeeSaleReportData' => $monthlySingleEmployeeSaleReportData,
            'monthlySingleEmployeeSaleProductReportData' => $monthlySingleEmployeeSaleProductReportData,
            'yearList' => $yearList,
            'year' => $year,
            'month' => $month,
    ...
    protected function saveToExcel($monthlySingleEmployeeSaleReportData, $monthlySingleEmployeeSaleProductReportData, $month, $year, $employeeId) {
        set_time_limit(0);
        ini_set('memory_limit', '1024M');
        ...
        for ($i = 1; $i <= 31; $i++) {
            if (isset($monthlySingleEmployeeSaleReportData[$i]) && isset($monthlySingleEmployeeSaleProductReportData[$i])) {
                $dataItem = $monthlySingleEmployeeSaleReportData[$i];
                $detailItem = $monthlySingleEmployeeSaleProductReportData[$i];
