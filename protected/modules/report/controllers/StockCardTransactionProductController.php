...
class StockCardTransactionProductController extends Controller {
    ...
    public function actionSummary() {
        ...
        $stockCardSummary = new StockCardTransactionProductSummary($product->searchByStockCard());
    ...
    protected function saveToExcel($stockCardSummary, $startDate, $endDate, $branchId) {
        ...
        $documentProperties->setTitle('Mutasi Stok Jual Beli Barang');

        $worksheet = $objPHPExcel->setActiveSheetIndex(0);
        $worksheet->setTitle('Mutasi Stok Jual Beli Barang');
        ...
        $worksheet->setCellValue('A2', 'Mutasi Stok Jual Beli Barang');
        ...
        foreach ($stockCardSummary->dataProvider->data as $header) {
            $stock = $header->getBeginningTransactionStockReport($startDate, $branchId); 
            $beginningValue = $header->getBeginningTransactionValueReport($startDate, $branchId);
            $stockData = $header->getInventoryTransactionStockReport($startDate, $endDate, $branchId); 
            $totalStockIn = 0;
            $totalStockOut = 0;
            ...
            foreach ($stockData as $stockRow) {
                ...
                $beginningStock = $stock;
                $beginningStockValue = $inventoryValue;
