...
class OutstandingSaleOrderController extends Controller {
...
//            'access',
    ...
    public function actionSummary() {
        ...
        $outstandingSaleOrderSummary = new OutstandingSaleOrderSummary($registrationTransaction->searchReport());
        $outstandingSaleOrderSummary->setupLoading();
        $outstandingSaleOrderSummary->setupPaging($pageSize, $currentPage);
        $outstandingSaleOrderSummary->setupSorting();
        $filters = array(
            'startDate' => $startDate,
            'endDate' => $endDate,
            'branchId' => $branchId,
        ...
        $outstandingSaleOrderSummary->setupFilter($filters);

        if (isset($_GET['SaveExcel'])) {
            $this->saveToExcel($outstandingSaleOrderSummary, $startDate, $endDate, $branchId);
        }

        $this->render('summary', array(
            'registrationTransaction' => $registrationTransaction,
            'outstandingSaleOrderSummary' => $outstandingSaleOrderSummary,
            'startDate' => $startDate,
            'endDate' => $endDate,
    ...
    protected function saveToExcel($outstandingSaleOrderSummary, $startDate, $endDate, $branchId) {
        set_time_limit(0);
        ini_set('memory_limit', '1024M');
        ...
        $documentProperties->setTitle('Outstanding Sales Order');

        $worksheet = $objPHPExcel->setActiveSheetIndex(0);
        $worksheet->setTitle('Outstanding Sales Order');
        ...
        $worksheet->setCellValue('A2', 'Outstanding Sales Order');
        ...
        $worksheet->setCellValue('B5', 'Sales Order #');
        ...
        foreach ($outstandingSaleOrderSummary->dataProvider->data as $i => $header) {
            ...
            $worksheet->setCellValue("B{$counter}", CHtml::value($header, 'sales_order_number'));
            $worksheet->setCellValue("C{$counter}", CHtml::value($header, 'transaction_number'));
            $worksheet->setCellValue("D{$counter}", CHtml::value($header, 'sales_order_date'));
        ...
        header('Content-Disposition: attachment;filename="outstanding_sale_order.xls"');
