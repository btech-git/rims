...
class CustomerVehicleSaleTransactionController extends Controller {
    ...
    protected function saveToExcel($saleFlowSummary, $startDate, $endDate, $branchId) {
        ...
        $worksheet->setCellValue('K5', 'Permintaan Awal / Keluhan');
        $worksheet->setCellValue('L5', 'Problem');
        $worksheet->setCellValue('M5', 'Salesman');
        $worksheet->setCellValue('N5', 'Mekanik');
        $worksheet->setCellValue('O5', 'KM Sebelum');
        $worksheet->setCellValue('P5', 'KM sekarang');
        $worksheet->setCellValue('Q5', 'KM Service Selanjutnya');
        $worksheet->setCellValue('R5', 'Rekomendasi Service Selanjutnya');
        $worksheet->setCellValue('S5', 'User Input');
        $worksheet->setCellValue('T5', 'Work Order');
        $worksheet->setCellValue('U5', 'Movement Out');
        $worksheet->setCellValue('V5', 'User Movement');
        $worksheet->setCellValue('W5', 'Invoice');
        $worksheet->setCellValue('X5', 'Tanggal Invoice');
        $worksheet->setCellValue('Y5', 'Jam');
        $worksheet->setCellValue('Z5', 'User Invoice');
        $worksheet->setCellValue('AA5', 'Payment In');
        $worksheet->setCellValue('AB5', 'Tanggal Payment');
        $worksheet->setCellValue('AC5', 'Jam');
        $worksheet->setCellValue('AD5', 'User Payment');
        ...
        foreach ($saleFlowSummary->dataProvider->data as $i => $header) {
            ...
            $worksheet->setCellValue("K{$counter}", CHtml::value($header, 'customer_request_note'));
            $worksheet->setCellValue("L{$counter}", CHtml::value($header, 'problem'));
            $worksheet->setCellValue("M{$counter}", CHtml::value($header, 'employeeIdSalesPerson.name'));
            $worksheet->setCellValue("N{$counter}", CHtml::value($header, 'employeeIdAssignMechanic.name'));
            $worksheet->setCellValue("O{$counter}", CHtml::value($header, 'previous_mileage'));
            $worksheet->setCellValue("P{$counter}", CHtml::value($header, 'vehicle_mileage'));
            $worksheet->setCellValue("Q{$counter}", CHtml::value($header, 'next_mileage'));
            $worksheet->setCellValue("R{$counter}", CHtml::value($header, 'next_service_recommendation'));
            $worksheet->setCellValue("S{$counter}", CHtml::value($header, 'user.username'));
            $worksheet->setCellValue("T{$counter}", CHtml::value($header, 'work_order_number'));
            $worksheet->setCellValue("U{$counter}", CHtml::encode(implode(', ', $movementOutHeaderCodeNumbers)));
            $worksheet->setCellValue("V{$counter}", CHtml::encode(implode(', ', $movementOutHeaderUsers)));
            $worksheet->setCellValue("W{$counter}", CHtml::encode(implode(', ', $invoiceHeaderCodeNumbers)));
            $worksheet->setCellValue("X{$counter}", CHtml::encode(implode(', ', $invoiceHeaderTransactionDates)));
            $worksheet->setCellValue("Y{$counter}", CHtml::encode(implode(', ', $invoiceHeaderTransactionTimes)));
            $worksheet->setCellValue("Z{$counter}", CHtml::encode(implode(', ', $invoiceHeaderUsers)));
            $worksheet->setCellValue("AA{$counter}", CHtml::encode(implode(', ', $paymentInHeaderCodeNumbers)));
            $worksheet->setCellValue("AB{$counter}", CHtml::encode(implode(', ', $paymentInHeaderDates)));
            $worksheet->setCellValue("AC{$counter}", CHtml::encode(implode(', ', $paymentInHeaderTimes)));
            $worksheet->setCellValue("AD{$counter}", CHtml::encode(implode(', ', $paymentInHeaderUsers)));
            $counter++;
        }
