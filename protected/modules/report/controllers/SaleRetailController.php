...
class SaleRetailController extends Controller {
    ...
    protected function saveToExcel($customerSaleReport, array $options = array()) {
        ...
        $worksheet->setCellValue('E5', 'Asuransi');
        ...
        foreach ($customerSaleReport as $i => $dataItem) {
            ...
            if (!empty($saleReportData)) {
                foreach ($saleReportData as $saleReportRow) {
                    ...
                    $worksheet->setCellValue("D{$counter}", $dataItem['customer_type']);
                    $worksheet->setCellValue("E{$counter}", CHtml::value($saleReportRow, 'insuranceCompany.name'));
