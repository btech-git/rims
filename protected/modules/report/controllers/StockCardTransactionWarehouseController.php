...
class StockCardTransactionWarehouseController extends Controller {
    ...
    public function actionSummary() {
        ...
        $stockCardSummary = new StockCardTransactionWarehouseSummary($product->searchByStockCard());
    ...
    protected function saveToExcel($stockCardSummary, $startDate, $endDate, $warehouseId) {
        ...
        $documentProperties->setTitle('Mutasi Stok Jual Beli Gudang');

        $worksheet = $objPHPExcel->setActiveSheetIndex(0);
        $worksheet->setTitle('Mutasi Stok Jual Beli Gudang');
        ...
        $worksheet->setCellValue('A2', 'Mutasi Stok Jual Beli Gudang ' . CHtml::value($warehouse, 'name'));
        ...
        foreach ($stockCardSummary->dataProvider->data as $header) {
            $stock = $header->getBeginningTransactionStockReport($startDate, $warehouse->branch_id);
            $beginningValue = $header->getBeginningValueReport($startDate, $warehouse->branch_id);
            $stockData = $header->getInventoryTransactionStockReport($startDate, $endDate, $warehouse->branch_id);
            $totalStockIn = 0;
            $totalStockOut = 0;
            $totalValueIn = 0;
            $totalValueOut = 0;
