...
class MinimumAndFastMovingProductController extends Controller {
    ...
    public function actionSummary() {
        ...
        $startMonth = intval($monthNow);
        $startYear = intval($yearNow);
        $endMonth = intval($monthNow);
        $endYear = intval($yearNow);
        for ($i = 0; $i < 6; $i++) {
            if ($i === 0) {
                $endMonth--;
                if ($endMonth === 0) {
        ...
        $endDate = $endYear . '-' . str_pad($endMonth, 2, '0', STR_PAD_LEFT) . '-' . $endDay;

        $minimumAndFastMovingProduct = Product::getMinimumAndFastMovingProductReport($startDate, $endDate);
        
        $productIds = array_map(function($minimumAndFastMovingProductItem) { return $minimumAndFastMovingProductItem['id']; }, $minimumAndFastMovingProduct);
        
        $inventoryCurrentStocks = InventoryDetail::getInventoryCurrentStocks($productIds, null);
        $fastMovingAverageProductQuantities = InvoiceDetail::getFastMovingAverageProductQuantities($productIds, $startDate, $endDate);
        
        $inventoryCurrentStocksData = array();
        foreach ($inventoryCurrentStocks as $inventoryCurrentStocksItem) {
            $inventoryCurrentStocksData[$inventoryCurrentStocksItem['product_id']] = $inventoryCurrentStocksItem['total_stock'];
        }
        
        $fastMovingAverageProductQuantitiesData = array();
        foreach ($fastMovingAverageProductQuantities as $fastMovingAverageProductQuantitiesItem) {
            $fastMovingAverageProductQuantitiesData[$fastMovingAverageProductQuantitiesItem['product_id']] = $fastMovingAverageProductQuantitiesItem['average_quantity'];
        }
