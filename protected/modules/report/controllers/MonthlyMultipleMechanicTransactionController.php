...
class MonthlyMultipleMechanicTransactionController extends Controller {
    ...
    public function filterAccess($filterChain) {
        if ($filterChain->action->id === 'summary') {
            if (!(Yii::app()->user->checkAccess('monthlySaleAllMechanicReport'))) {
    ...
    public function actionSummary() {
        ...
        foreach ($monthlyMultipleMechanicTransactionServiceReport as $monthlyMultipleMechanicTransactionServiceReportItem) {
            $monthlyMultipleMechanicTransactionServiceReportData[$monthlyMultipleMechanicTransactionServiceReportItem['employee_id_assign_mechanic']] = $monthlyMultipleMechanicTransactionServiceReportItem;
        }
        ...
        if (isset($_GET['SaveExcel'])) {
            $this->saveToExcel($monthlyMultipleMechanicTransactionReport, $monthlyMultipleMechanicTransactionServiceReportData, $month, $year);
        }
        
        $this->render('summary', array(
            'monthlyMultipleMechanicTransactionReport' => $monthlyMultipleMechanicTransactionReport,
            'monthlyMultipleMechanicTransactionServiceReportData' => $monthlyMultipleMechanicTransactionServiceReportData,
            'yearList' => $yearList,
            'year' => $year,
            'month' => $month,
        ));
    ...
    public function actionTransactionInfo($mechanicId, $month, $year) {
        set_time_limit(0);
        ini_set('memory_limit', '1024M');

        $page = (isset($_GET['page'])) ? $_GET['page'] : 1;
        
        $dataProvider = InvoiceHeader::model()->searchByMechanicMonthlyTransactionInfo($mechanicId, $year, $month, $page);
    ...
    protected function saveToExcel($monthlyMultipleMechanicTransactionReport, $monthlyMultipleMechanicTransactionServiceReportData, $month, $year) {
        set_time_limit(0);
        ini_set('memory_limit', '1024M');
        ...
        foreach ($monthlyMultipleMechanicTransactionReport as $dataItem) {
            $detailItem = $monthlyMultipleMechanicTransactionServiceReportData[$dataItem['employee_id_assign_mechanic']];
