...
//            'access',
    ...
    public function actionSummary() {
        ...
        foreach ($coas as $coa) {
            $workingSheetReportData[$coa->id]['cashflow_position'] = $coa->coaSubCategory->cashflow_position;
        ...
        foreach ($workingSheetBeginningBalances as $workingSheetBeginningBalanceItem) {
            if ($workingSheetReportData[$workingSheetBeginningBalanceItem['coa_id']]['cashflow_position'] !== 'Laba Bersih') {
        ...
        foreach ($coas as $coa) {
            $workingSheetReportData[$coa->id]['positive_debit_total'] = abs($workingSheetReportData[$coa->id]['debit_total']);
            $workingSheetReportData[$coa->id]['positive_credit_total'] = abs($workingSheetReportData[$coa->id]['credit_total']);
            $workingSheetReportData[$coa->id]['ending_balance'] = $workingSheetReportData[$coa->id]['beginning_balance'] + $workingSheetReportData[$coa->id]['debit_total'] + $workingSheetReportData[$coa->id]['credit_total'];
            if ($workingSheetReportData[$coa->id]['cashflow_position'] !== 'Laba Bersih') {
                $workingSheetReportData[$coa->id]['balance_difference'] = $workingSheetReportData[$coa->id]['ending_balance'] - $workingSheetReportData[$coa->id]['beginning_balance'];
            }
            if ($workingSheetReportData[$coa->id]['cashflow_position'] === 'Laba Bersih') {
                $workingSheetReportData[$coa->id]['profit_loss_balance'] = $workingSheetReportData[$coa->id]['positive_credit_total'] - $workingSheetReportData[$coa->id]['positive_debit_total'];
            }
        }
