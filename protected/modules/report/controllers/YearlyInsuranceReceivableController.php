...
class YearlyInsuranceReceivableController extends Controller {
    ...
    public function actionSummary() {
        ...
        foreach ($yearlyInsuranceInvoiceReport as $yearlyInsuranceInvoiceReportItem) {
            $yearlyInsuranceReport[$yearlyInsuranceInvoiceReportItem['insurance_company_id']][$yearlyInsuranceInvoiceReportItem['invoice_month']]['invoice_total'] = $yearlyInsuranceInvoiceReportItem['invoice_total'];
            $yearlyInsuranceReport[$yearlyInsuranceInvoiceReportItem['insurance_company_id']]['insurance_name'] = $yearlyInsuranceInvoiceReportItem['insurance_name'];
        }
        
        foreach ($yearlyInsurancePaymentReport as $yearlyInsurancePaymentReportItem) {
            $yearlyInsuranceReport[$yearlyInsurancePaymentReportItem['insurance_company_id']][$yearlyInsurancePaymentReportItem['payment_month']]['payment_total'] = $yearlyInsurancePaymentReportItem['payment_total'];
            $yearlyInsuranceReport[$yearlyInsurancePaymentReportItem['insurance_company_id']]['insurance_name'] = $yearlyInsurancePaymentReportItem['insurance_name'];
        }
        
        foreach ($beginningInsuranceInvoiceReport as $beginningInsuranceInvoiceReportItem) {
            $yearlyInsuranceReport[$beginningInsuranceInvoiceReportItem['insurance_company_id']]['beginning_invoice_total'] = $beginningInsuranceInvoiceReportItem['beginning_invoice_total'];
            $yearlyInsuranceReport[$beginningInsuranceInvoiceReportItem['insurance_company_id']]['insurance_name'] = $beginningInsuranceInvoiceReportItem['insurance_name'];
        }
        
        foreach ($beginningInsurancePaymentReport as $beginningInsurancePaymentReportItem) {
            $yearlyInsuranceReport[$beginningInsurancePaymentReportItem['insurance_company_id']]['beginning_payment_total'] = $beginningInsurancePaymentReportItem['beginning_payment_total'];
            $yearlyInsuranceReport[$beginningInsurancePaymentReportItem['insurance_company_id']]['insurance_name'] = $beginningInsurancePaymentReportItem['insurance_name'];
        }
        
        $yearlyInsuranceReportData = array();
        foreach ($yearlyInsuranceReport as $insuranceId => $yearlyInsuranceReportItem) {
            $beginningInvoiceTotal = isset($yearlyInsuranceReportItem['beginning_invoice_total']) ? $yearlyInsuranceReportItem['beginning_invoice_total'] : '0.00';
            $beginningPaymentTotal = isset($yearlyInsuranceReportItem['beginning_payment_total']) ? $yearlyInsuranceReportItem['beginning_payment_total'] : '0.00';
            ...
            for ($month = 1; $month <= 12; $month++) {
                $invoiceTotal = isset($yearlyInsuranceReportItem[$month]['invoice_total']) ? $yearlyInsuranceReportItem[$month]['invoice_total'] : '0.00';
                $paymentTotal = isset($yearlyInsuranceReportItem[$month]['payment_total']) ? $yearlyInsuranceReportItem[$month]['payment_total'] : '0.00';
            ...
            if (!$allOustandingEmpty) {
                $yearlyInsuranceReportData[$insuranceId] = $yearlyInsuranceReportItem;
            }
        }
        ...
        if (isset($_GET['SaveExcel'])) {
            $this->saveToExcel($yearlyInsuranceReportData, $year);
        }
    ...
    public function actionTransactionInfo($insuranceId, $year, $month) {
        set_time_limit(0);
        ini_set('memory_limit', '1024M');

        $insuranceCompany = InsuranceCompany::model()->findByPk($insuranceId);
        $dataProvider = JurnalUmum::model()->searchByYearlyInsuranceReceivableInfo($insuranceCompany->coa_id, $year, $month);
        $beginningBalance = JurnalUmum::model()->getYearlyInsuranceReceivableBeginningBalance($insuranceCompany->coa_id, $year, $month);
        ...
        if (isset($_GET['SaveExcel'])) {
            $this->saveToExcelTransactionInfo($dataProvider, $year, $month, $monthList, $insuranceCompany);
        }
    ...
    protected function saveToExcel($yearlyInsuranceReportData, $year) {
        set_time_limit(0);
        ini_set('memory_limit', '1024M');
        ...
        $documentProperties->setTitle('Piutang Asuransi Tahunan');

        $worksheet = $objPHPExcel->setActiveSheetIndex(0);
        $worksheet->setTitle('Piutang Asuransi Tahunan');
        ...
        foreach ($yearlyInsuranceReportData as $yearlyInsuranceReportDataItem) {
            $worksheet->getStyle("E{$counter}:J{$counter}")->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);

            $beginningInvoiceTotal = isset($yearlyInsuranceReportDataItem['beginning_invoice_total']) ? $yearlyInsuranceReportDataItem['beginning_invoice_total'] : '0.00';
            $beginningPaymentTotal = isset($yearlyInsuranceReportDataItem['beginning_payment_total']) ? $yearlyInsuranceReportDataItem['beginning_payment_total'] : '0.00';
            $beginningOutstanding = $beginningInvoiceTotal - $beginningPaymentTotal; 
            
            $worksheet->setCellValue("A{$counter}", ++$ordinal);
            $worksheet->setCellValue("B{$counter}", $yearlyInsuranceReportDataItem['insurance_name']);
            ...
            for ($month = 1; $month <= 12; $month++) {
                $invoiceTotal = isset($yearlyInsuranceReportDataItem[$month]['invoice_total']) ? $yearlyInsuranceReportDataItem[$month]['invoice_total'] : '0.00';
                $paymentTotal = isset($yearlyInsuranceReportDataItem[$month]['payment_total']) ? $yearlyInsuranceReportDataItem[$month]['payment_total'] : '0.00';
        ...
        header('Content-Disposition: attachment;filename="piutang_asuransi_tahunan.xls"');
    ...
    protected function saveToExcelTransactionInfo($dataProvider, $year, $month, $monthList, $insuranceCompany) {
        set_time_limit(0);
        ini_set('memory_limit', '1024M');
        ...
        $documentProperties = $objPHPExcel->getProperties();
        $documentProperties->setCreator('Raperind Motor');
        $documentProperties->setTitle('Piutang Asuransi ' . $monthList[$month] . ' ' . $year);

        $worksheet = $objPHPExcel->setActiveSheetIndex(0);
        $worksheet->setTitle('Piutang Asuransi ' . $monthList[$month] . ' ' . $year);
        ...
        $worksheet->setCellValue('A1', 'Raperind Motor ');
        $worksheet->setCellValue('A2', 'Piutang Asuransi ' . CHtml::value($customer, 'name'));
