...
class WorkOrderByVehicleController extends Controller {
...
//        $service = Search::bind(new Service('search'), isset($_GET['Service']) ? $_GET['Service'] : '');
        ...
        $workOrderVehicleSummary = new WorkOrderByVehicleSummary($vehicle->search());
        $workOrderVehicleSummary->setupLoading();
        $workOrderVehicleSummary->setupPaging($pageSize, $currentPage);
        $workOrderVehicleSummary->setupSorting();
        $filters = array(
            'startDate' => $startDate,
            'endDate' => $endDate,
        );
        $workOrderVehicleSummary->setupFilter($filters);

//        if (isset($_GET['SaveExcel'])) {
//            $this->saveToExcel($workOrderVehicleSummary, $startDate, $endDate);
//        }
    ...
    protected function saveToExcel($workOrderVehicleSummary, $startDate, $endDate) {
        set_time_limit(0);
        ini_set('memory_limit', '1024M');
        ...
        foreach ($workOrderVehicleSummary->dataProvider->data as $header) {
