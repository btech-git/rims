...
<table class="report">
    <tr id="header1">
        ...
        <th class="width1-8">Service</th>
        <th class="width1-9">Price</th>
        <th class="width1-10">Disc</th>
    ...
    <?php foreach ($saleRetailSummary->dataProvider->data as $header): ?>
        <?php foreach ($header->registrationServices as $detail): ?>
            <tr class="items1">
                ...
                <td class="width1-8"><?php echo CHtml::encode(CHtml::value($detail, 'service.name')); ?></td>
                <td class="width1-9"><?php echo CHtml::encode(Yii::app()->numberFormatter->format('#,##0', CHtml::value($detail, 'price'))); ?></td>
                <td class="width1-10"><?php echo CHtml::encode(Yii::app()->numberFormatter->format('#,##0', CHtml::value($detail, 'discount'))); ?></td>
                <td class="width1-11"><?php echo CHtml::encode(Yii::app()->numberFormatter->format('#,##0', CHtml::value($detail, 'total_price'))); ?></td>
            </tr>
        <?php endforeach; ?>
    <?php endforeach; ?>
</table>