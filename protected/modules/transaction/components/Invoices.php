...
class Invoices extends CComponent {
    ...
    public function saveSaleOrder($dbConnection) {
        $dbTransaction = $dbConnection->beginTransaction();
        try {
            $valid = $this->validate() && IdempotentManager::build()->save() && $this->flushSaleOrder();
            if ($valid) {
                $dbTransaction->commit();
            } else {
                $dbTransaction->rollback();
    ...
    public function flushSaleOrder() {
    ...
    public function saveTransactionLog() {
        ...
        $transactionLog->transaction_number = $this->header->invoice_number;
        $transactionLog->transaction_date = $this->header->invoice_date;
