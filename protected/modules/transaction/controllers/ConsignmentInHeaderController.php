...
class ConsignmentInHeaderController extends Controller {
    ...
    public function filterAccess($filterChain) {
        if ($filterChain->action->id === 'create') {
            if (!(Yii::app()->user->checkAccess('consignmentInCreate')))
        ...
        if (
            ...
            if (!(Yii::app()->user->checkAccess('consignmentInEdit')))
                $this->redirect(array('/site/login'));
        }

        if ($filterChain->action->id === 'updateApproval') {
            if (!(Yii::app()->user->checkAccess('consignmentInApproval')))
                $this->redirect(array('/site/login'));
        }
        ...
        if (
            ...
            if (!(Yii::app()->user->checkAccess('consignmentInCreate')) || !(Yii::app()->user->checkAccess('consignmentInEdit')) || !(Yii::app()->user->checkAccess('consignmentInApproval')))
                $this->redirect(array('/site/login'));
        }
    ...
    public function actionCreate() {
        ...
        $consignmentIn->header->date_created = date('Y-m-d H:i:s');
