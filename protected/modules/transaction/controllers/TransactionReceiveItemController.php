...
class TransactionReceiveItemController extends Controller {
    ...
    public function filterAccess($filterChain) {
        if ($filterChain->action->id === 'create' || $filterChain->action->id === 'addInvoice') {
        ...
        if (
            ...
            $filterChain->action->id === 'showInvoice' || 
...
//            'movementDataProvider' => $movementDataProvider,
    ...
    public function actionPdfInvoice($id) {
        ...
        $stylesheet = file_get_contents(Yii::getPathOfAlias('webroot') . '/css/pdf.css');
        $mPDF1->SetTitle('Supplier Invoice');
        $mPDF1->WriteHTML($stylesheet, 1);
        $mPDF1->WriteHTML($this->renderPartial('pdfInvoice', array(
            'receiveItem' => $receiveItem,
        ), true));
        $mPDF1->Output('Supplier Invoice #' . $receiveItem->invoice_number . '.pdf', 'I');
    }
