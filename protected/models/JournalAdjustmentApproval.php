...
/**
 * This is the model class for table "{{journal_adjustment_approval}}".
 *
 * The followings are the available columns in table '{{journal_adjustment_approval}}':
 * @property integer $id
 ...
 */
class JournalAdjustmentApproval extends CActiveRecord
{
	/**
	...
	{
		return '{{journal_adjustment_approval}}';
	}
	...
	public function rules()
	{
		...
		return array(
			array('approval_type, date, time, journal_adjustment_header_id, supervisor_id', 'required'),
			array('revision, journal_adjustment_header_id, supervisor_id', 'numerical', 'integerOnly'=>true),
			array('approval_type', 'length', 'max'=>50),
			array('note', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, revision, approval_type, date, time, note, journal_adjustment_header_id, supervisor_id', 'safe', 'on'=>'search'),
		);
	}
	 ...
	 * @return JournalAdjustmentApproval the static model class
	 */
