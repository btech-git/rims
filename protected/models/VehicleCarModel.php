...
class VehicleCarModel extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('name, car_make_id, status', 'required'),
			array('car_make_id', 'numerical', 'integerOnly'=>true),
			array('name', 'length', 'max'=>30),
			array('description', 'length', 'max'=>60),
			array('status', 'length', 'max'=>10),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, name, description, car_make_id, car_make, status', 'safe', 'on'=>'search'),
		);
	}
