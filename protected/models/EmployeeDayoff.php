...
class EmployeeDayoff extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('employee_id, day, notes, date_from, date_to, off_type', 'required'),
			array('employee_id', 'numerical', 'integerOnly'=>true),
			array('day', 'numerical'),
			array('status, off_type', 'length', 'max'=>30),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, employee_id, day, notes, date_from, date_to, status, off_type,employee_name', 'safe', 'on'=>'search'),
		);
	}
