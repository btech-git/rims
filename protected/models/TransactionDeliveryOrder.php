...
//            'user' => array(self::BELONGS_TO, 'Users', 'sender_id'),
    ...
    public function searchByReceive() {
        ...
        $criteria->condition = "EXISTS (
            SELECT d.delivery_order_id, COALESCE(SUM(d.quantity_receive_left), 0) AS quantity_remaining, COALESCE(SUM(d.quantity_movement_left), 0) AS quantity_movement_remaining
    ...
    public static function pendingJournal() {
        $sql = "SELECT p.id, p.delivery_order_no, p.delivery_date, b.name as branch_name, p.request_type
                ...
                WHERE p.delivery_date > '2023-12-31' AND p.delivery_order_no NOT IN (
