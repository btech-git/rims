...
//        $criteria->compare('LOWER(status)', strtolower($this->status), FALSE);
    ...
    public static function getServiceTransactionCountData($year, $month, $branchId) {
        ...
        $sql = "SELECT DATE(h.transaction_date) AS transaction_date, COUNT(DISTINCT h.transaction_number, s.service_id) AS service_count
                ...
                WHERE YEAR(h.transaction_date) = :year AND MONTH(h.transaction_date) = :month AND h.repair_type = 'BR' AND h.user_id_cancelled IS NULL" . $branchConditionSql . " 
