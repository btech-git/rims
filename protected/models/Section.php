...
/**
 * This is the model class for table "rims_section".
 *
 * The followings are the available columns in table 'rims_section':
 * @property integer $id
...
class Section extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('rack_number, column, row', 'required'),
			array('code, rack_number, column, row', 'length', 'max'=>20),
			array('status', 'length', 'max'=>10),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, code, rack_number, column, row, status', 'safe', 'on'=>'search'),
		);
	}
