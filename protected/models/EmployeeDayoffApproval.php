...
/**
 * This is the model class for table "{{employee_dayoff_approval}}".
 *
 * The followings are the available columns in table '{{employee_dayoff_approval}}':
 * @property integer $id
 ...
 */
class EmployeeDayoffApproval extends CActiveRecord
{
	/**
	...
	{
		return '{{employee_dayoff_approval}}';
	}
	...
	public function rules()
	{
		...
		return array(
			array('employee_dayoff_id, revision, approval_type, date, supervisor_id, note', 'required'),
			array('employee_dayoff_id, revision, supervisor_id', 'numerical', 'integerOnly'=>true),
			array('approval_type', 'length', 'max'=>30),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, employee_dayoff_id, revision, approval_type, date, supervisor_id, note', 'safe', 'on'=>'search'),
		);
	}
	 ...
	 * @return EmployeeDayoffApproval the static model class
	 */
