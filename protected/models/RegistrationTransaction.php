...
//    }
    ...
    public static function getSaleReportByProductCategory($startDate, $endDate, $branchId) {
        ...
        $sql = "SELECT c.customer_type, DATE(h.transaction_date) AS transaction_date, p.product_master_category_id, SUM(d.total_price) AS total_price
                FROM " . RegistrationTransaction::model()->tableName() . " h 
                INNER JOIN " . RegistrationProduct::model()->tableName() . " d ON h.id = d.registration_transaction_id
                INNER JOIN " . Customer::model()->tableName() . " c ON c.id = h.customer_id
                INNER JOIN " . Product::model()->tableName() . " p ON p.id = d.product_id
                WHERE DATE(h.transaction_date) BETWEEN :start_date AND :end_date AND h.status NOT LIKE '%CANCEL%'" . $branchConditionSql . "
    ...
    public static function getSaleReportByServiceType($startDate, $endDate, $branchId) {
        ...
        $sql = "SELECT c.customer_type, DATE(h.transaction_date) AS transaction_date, d.service_type_id, SUM(d.total_price) AS total_price
                FROM " . RegistrationTransaction::model()->tableName() . " h 
                INNER JOIN " . RegistrationService::model()->tableName() . " d ON h.id = d.registration_transaction_id
                INNER JOIN " . Customer::model()->tableName() . " c ON c.id = h.customer_id
                WHERE DATE(h.transaction_date) BETWEEN :start_date AND :end_date AND h.status NOT LIKE '%CANCEL%'" . $branchConditionSql . "
