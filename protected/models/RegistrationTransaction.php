 ...
 * @property string $customer_request_note
 * @property string $downpayment_updated_datetime
 * @property integer $user_id_updated_downpayment
 *
 ...
 * @property Users $userIdUpdatedDownpayment
 */
class RegistrationTransaction extends MonthlyTransactionActiveRecord {
    ...
    public function relations() {
        ...
        return array(
            ...
            'userIdUpdatedDownpayment' => array(self::BELONGS_TO, 'Users', 'user_id_updated_downpayment'),
        );
    }
    ...
    public function attributeLabels() {
        return array(
            ...
            'downpayment_updated_datetime' => 'Downpayment Updated Datetime',
            'user_id_updated_downpayment' => 'User Id Updated Downpayment',
        );
    }
    ...
    public function searchByVehicleSystemCheck() {
        ...
        $criteria->condition = "NOT EXISTS (
            SELECT registration_transaction_id
            FROM " . VehicleSystemCheckHeader::model()->tableName() . "
            WHERE t.id = registration_transaction_id
        ) AND t.total_service > 0 AND t.transaction_date > '" . AppParam::BEGINNING_TRANSACTION_DATE . "'";
    ...
    public function searchByReceiveParts() {
        $criteria = new CDbCriteria;
        ...
        $criteria->compare('t.problem', $this->problem, true);
        $criteria->compare('t.customer_id', $this->customer_id);
        $criteria->compare('t.vehicle_id', $this->vehicle_id);
        $criteria->compare('t.branch_id', $this->branch_id);
        $criteria->compare('t.user_id', $this->user_id);
        ...
        $criteria->compare('t.work_order_date', $this->work_order_date, true);
        $criteria->compare('t.status', $this->status, true);
        $criteria->compare('t.vehicle_mileage', $this->vehicle_mileage, true);
        $criteria->compare('t.note', $this->note, true);
        $criteria->compare('t.customer_work_order_number', $this->customer_work_order_number);
    ...
    public static function getActiveWorkOrderData($branchId, $limit, $startDate, $endDate, $plateNumber, $carMakeId, $carModelId, $workOrderNumber, $transactionStatus, $repairType, $customerName) {
        ...
        $sql = "SELECT r.vehicle_id, v.plate_number, r.transaction_date, c.name AS car_make, m.name AS car_model, s.name AS car_sub_model, o.name AS color,
                ...
                FROM " . RegistrationTransaction::model()->tableName() . " r
                INNER JOIN " . Customer::model()->tableName() . " cu on cu.id = r.customer_id
                INNER JOIN " . Branch::model()->tableName() . " b on b.id = r.branch_id
                INNER JOIN " . Vehicle::model()->tableName() . " v on v.id = r.vehicle_id
                INNER JOIN " . VehicleCarMake::model()->tableName() . " c on c.id = v.car_make_id
                INNER JOIN " . VehicleCarModel::model()->tableName() . " m on m.id = v.car_model_id
    ...
    public static function getOutstandingRegistrationData($branchId, $limit, $startDate, $endDate, $plateNumber, $carMakeId, $carModelId, $customerName, $repairType) {
        ...
        $sql = "SELECT r.vehicle_id, v.plate_number, r.transaction_date, c.name AS car_make, m.name AS car_model, s.name AS car_sub_model, o.name AS color,
                    r.customer_work_order_number, r.repair_type, r.problem, u.username, r.status, r.id, r.transaction_number, cu.name AS customer_name,
                    r.estimate_discharge_date
                FROM " . RegistrationTransaction::model()->tableName() . " r
                INNER JOIN " . Customer::model()->tableName() . " cu on cu.id = r.customer_id
                INNER JOIN " . Branch::model()->tableName() . " b on b.id = r.branch_id
                INNER JOIN " . Vehicle::model()->tableName() . " v on v.id = r.vehicle_id
    ...
    public static function getRegistrationTransactionCountData($year, $month, $branchId) {
        $branchConditionSql = '';
        ...
        $sql = "SELECT DATE(transaction_date) AS transaction_date, COUNT(DISTINCT transaction_number) AS transaction_count
                FROM " . RegistrationTransaction::model()->tableName() . " 
                WHERE YEAR(transaction_date) = :year AND MONTH(transaction_date) = :month AND repair_type = 'BR' AND user_id_cancelled IS NULL" . $branchConditionSql . " 
                GROUP BY DATE(transaction_date)";
    ...
    public static function getRegistrationWorkOrderCountData($year, $month, $branchId) {
        $branchConditionSql = '';
        ...
        $sql = "SELECT DATE(transaction_date) AS transaction_date, COUNT(DISTINCT work_order_number) AS work_order_count
                FROM " . RegistrationTransaction::model()->tableName() . " 
                WHERE YEAR(transaction_date) = :year AND MONTH(transaction_date) = :month AND repair_type = 'BR' AND user_id_cancelled IS NULL AND 
                    work_order_number IS NOT NULL" . $branchConditionSql . " 
                GROUP BY DATE(transaction_date)";
    ...
    public function getReceivableAmount() {
        $saleInvoiceInsuranceOwnRisk = SaleInvoiceInsuranceOwnRisk::model()->findByAttributes(array(
            'registration_transaction_id' => $this->id, 
            'user_id_cancelled' => null,
        ));
        
        $ownRiskAmount = !empty($saleInvoiceInsuranceOwnRisk) ? $saleInvoiceInsuranceOwnRisk->amount_invoice : '0.00';
