...
class RegistrationTransaction extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('customer_id, pic_id, vehicle_id, branch_id, user_id, total_quickservice, total_service, is_quick_service, is_insurance, insurance_company_id, laststatusupdate_by', 'numerical', 'integerOnly'=>true),
	...
	public function relations()
	{
		...
		return array(
			...
			'workOrders' => array(self::HAS_MANY, 'WorkOrder', 'registration_number_id'),
		);
	}
	...
	public function search()
	{
		...
		$criteria->mergeWith($this->dateRangeSearchCriteria('transaction_date', $this->transaction_date,true));  
		...
		//$criteria->compare('work_order_number !=""',true, 'OR');
		...
		$criteria->with = array('vehicle'=>array('with'=>array('carMake','carModel')),'customer','pic');
    ...
    function getColor($data, $row) {
		$color = '';
    	if ($data->vehicle->color_id) {
			$colors = Colors::model()->findByPk($data->vehicle->color_id);
				if($colors['name'])
					$color = $colors['name'];
