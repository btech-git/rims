 ...
 * @property string $technical_code_number
 * @property string $warranty_report
 * @property string $delivery_receipt_number
 *
 ...
 * @property UserIdCoretax $userIdCoretax
 */
class InvoiceHeader extends MonthlyTransactionActiveRecord {
    ...
    public function rules() {
        ...
        return array(
            ...
            array('invoice_number, transaction_tax_number, coretax_receipt_number, delivery_receipt_number', 'length', 'max' => 60),
            array('status', 'length', 'max' => 30),
            array('technical_code_number', 'length', 'max' => 100),
    ...
    public function relations() {
        ...
        return array(
            ...
            'userIdCoretax' => array(self::BELONGS_TO, 'Users', 'user_id_coretax'),
        );
    }
    ...
    public function searchForPaymentIn() {
        ...
        $criteria->together = 'true';
        $criteria->with = array('customer', 'vehicle', 'insuranceCompany');
        $criteria->addSearchCondition('vehicle.plate_number', $this->plate_number, true);
        $criteria->addSearchCondition('customer.name', $this->customer_name, true);
        $criteria->addSearchCondition('insuranceCompany.name', $this->insurance_company_name, true);
    ...
    public function searchForSaleReceipt() {
        ...
        $criteria->condition = "NOT EXISTS (
            SELECT d.invoice_header_id 
            FROM " . SaleReceiptDetail::model()->tableName() . " d
            INNER JOIN " . SaleReceiptHeader::model()->tableName() . " h ON h.id = d.sale_receipt_header_id
            WHERE h.user_id_cancelled IS NULL
        )";
    ...
    public function getRemainingInvoiceAmount() {
        return $this->total_price - $this->downpayment_amount;
    }
    ...
    public static function getDailyMultipleMechanicTransactionReport($startDate, $endDate) {
        ...
        $sql = "SELECT r.employee_id_assign_mechanic, MAX(e.name) AS employee_name, COUNT(h.vehicle_id) AS vehicle_quantity, 
                    COUNT(CASE WHEN c.customer_type = 'Individual' THEN 1 END) AS customer_retail_quantity, COUNT(r.work_order_number) AS work_order_quantity, 
                    COUNT(CASE WHEN c.customer_type = 'Company' THEN 1 END) AS customer_company_quantity, SUM(h.service_price) AS total_service
                FROM " . InvoiceHeader::model()->tableName() . " h 
                INNER JOIN " . RegistrationTransaction::model()->tableName() . " r ON r.id = h.registration_transaction_id
    ...
    public static function getMonthlyMultipleMechanicTransactionReport($year, $month) {
        ...
        $sql = "SELECT r.employee_id_assign_mechanic, MAX(e.name) AS employee_name, COUNT(h.vehicle_id) AS vehicle_quantity, 
                ...
                INNER JOIN " . Employee::model()->tableName() . " e ON e.id = r.employee_id_assign_mechanic
                INNER JOIN " . Customer::model()->tableName() . " c ON c.id = h.customer_id
                WHERE YEAR(h.invoice_date) = :year AND MONTH(h.invoice_date) = :month AND r.employee_id_assign_mechanic IS NOT NULL AND 
                    h.status NOT LIKE '%CANCEL%' AND r.status NOT LIKE '%CANCEL%'
                GROUP BY r.employee_id_assign_mechanic
                ORDER BY MAX(e.name) ASC";
    ...
    public function searchByCustomerSaleTransactionInfo($customerId, $branchId, $startDate, $endDate, $page) {
...
//    }
    ...
    public static function getReceivableReport($endDate, $branchId, $customerIds) {
    ...
    public static function getSaleInvoiceVehicleReport($startDate, $endDate, $branchId, $carSubModelIds) {
        $carSubModelIdsSql = empty($carSubModelIds) ? 'NULL' : implode(',', $carSubModelIds);
        $branchConditionSql = '';
