...
/**
 * This is the model class for table "{{invoice_approval}}".
 *
 * The followings are the available columns in table '{{invoice_approval}}':
 * @property integer $id
 ...
 */
class InvoiceApproval extends CActiveRecord
{
	/**
	...
	public function rules()
	{
		...
		return array(
			array('revision, approval_type, date, time, invoice_header_id, supervisor_id', 'required'),
			array('revision, invoice_header_id, supervisor_id', 'numerical', 'integerOnly'=>true),
			array('approval_type', 'length', 'max'=>30),
			array('note', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, revision, approval_type, date, time, note, invoice_header_id, supervisor_id', 'safe', 'on'=>'search'),
		);
	}
	 ...
	 * @return InvoiceApproval the static model class
	 */
