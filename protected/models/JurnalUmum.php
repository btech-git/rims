...
//    }
    ...
    public static function getAssetJournalData($startDate, $endDate, $branchId) {
        $branchConditionSql = '';
        ...
        $sql = "SELECT j.coa_id AS coa_id, c.code AS coa_code, c.name AS coa_name, SUM(IF(j.debet_kredit = 'D', j.total, 0)) AS debit, 
                ...
                ) AND j.tanggal_transaksi BETWEEN :start_date AND :end_date AND j.transaction_type IN ('PFA', 'DFA', 'SFA') AND j.is_coa_category = 0 " . 
                    $branchConditionSql . "
                GROUP BY j.coa_id
                ORDER BY c.code ASC";
    ...
    public static function getBeginningBalanceDataByTransactionYear($startYearMonth, $branchId) {
        ...
        $sql = "SELECT j.coa_id, COALESCE(SUM(
                ...
                FROM " . JurnalUmum::model()->tableName() . " j
                INNER JOIN " . Coa::model()->tableName() . " c ON c.id = j.coa_id
                WHERE SUBSTRING_INDEX(j.tanggal_transaksi, '-', 2) < :start_year_month AND tanggal_transaksi >= '" . AppParam::BEGINNING_TRANSACTION_DATE . "' 
                    AND c.coa_category_id IN (4, 5, 14, 15, 16, 17, 18, 19, 20, 21, 23) AND c.is_approved = 1" . $branchConditionSql . "
                GROUP BY j.coa_id";
